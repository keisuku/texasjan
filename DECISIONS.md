...
| 2026-08-19 | ACCEPTED | 「一旦ハリボテ」は、低品質なCSSプレースホルダーを許容する意味ではなく、承認済みGolden Visualを前面に使い操作だけ実物として通すimage-first仮実装を指す。 | タイトル、ロビー、モード選択、装飾品、ガチャは既存の評価済み画像へ差し替える。最終実装では画像を背景・人物・UIへ分解する。 |
| 2026-08-19 | REJECTED | 暗い空白、立ち絵の仮枠、無地の色見本カード、弱いCSSグラデーションを「華やかなプチ完成版」として本番表示する。 | 機能が動いても製品価値を損なうため、メタ画面の表示面から退役させる。 |
| 2026-08-19 | WORKING BASELINE | ハリボテ解消は、Golden Visualを一度に捨てず、人物・空間をキーアートとして保護しながら焼き込み操作を日本語の実DOMへ置換する。 | 第1段階としてロビー／モード選択の透明ホットスポットを撤去。通貨・ランク・モードカード・ナビを実データ／実ボタン化した。 |
| 2026-08-19 | WORKING BASELINE | 装飾品とガチャも、キーアートの品質を保護しながら選択・通貨・抽選・獲得結果を実DOMへ分離する。 | 固定されたガチャ結果絵を操作面として使わず、実際に抽選した報酬名、重複変換、再抽選を表示する。 |
| 2026-08-19 | WORKING BASELINE | 対局・結果の表示は日本語を主にし、COMMON / PRIVATE / YOUR TURN / THINKING等の内部英語を操作面へ残さない。 | 共通牌、私牌、最終手牌、あなたの番、考え中、降りる／コール／レイズ／オールインへ統一。ブランド名とPOTは維持する。 |
| 2026-08-19 | WORKING BASELINE | ガチャと装飾品一覧には、実プレビューと対局反映を確認できる完成済み商品だけを出す。 | 完成済み4種をreadyとして排出。黒曜・夜想・翡翠チップ・黒檀など素材未完成の予約枠は非表示・非排出。 |
| 2026-08-19 | WORKING BASELINE | 強い演出は実データに連動し、オールインと勝利へ局所的に出す。 | ALL-INアクション成立時だけ中央演出。実勝利時だけ結果ヒーローへ和了FXを表示し、平常時は明滅させない。 |

| 2026-08-20 | REJECTED | PR #43にマージされた29画面のビジュアルを、承認済み候補・再利用元・完成方向として扱う。 | 人間が全案を明示却下。main上のファイルは履歴資料であり、マージ済みでも採用を意味しない。 |
| 2026-08-20 | REJECTED | PR #44の対局画面案を使用または継続する。 | 画風探索をゲーム構造・カメラ角度の奇抜化と誤読したため、未マージのまま閉鎖。 |
| 2026-08-20 | REJECTED | PR #45のキャラクター画風R6を使用または継続する。 | `audition-r5`の意図を再現できず、未マージのまま閉鎖。 |
| 2026-08-20 | ACCEPTED | 新セッションは `NEW_SESSION_START_HERE.md` から入り、実装済み／未承認／不採用／未解決を確認してから作業する。 | 巨大な会話ログではなく、GitHubの短い状態ファイルを正典にする。次の指示前に生成・変更・マージを始めない。 |

| 2026-09-07 | ACCEPTED | ユーザーがtexasjan再開、ゼロからの抜本的再検討、大至急の実装を明示指示。 | 当該指示により次指示待ちは解除。既存の不採用ビジュアルを承認扱いにするものではない。 |
| 2026-09-07 | WORKING BASELINE | 第一段階は一局の整合・判断支援・振り返りへ集中する。 | サイドPOT、固定牌遵守、手牌探索、初回ガイド、速度、端末記録、同じ配牌の練習を実装。docs/RESTART_2026-09-07.md |
| 2026-09-07 | WORKING BASELINE | 既存の比例参加料は共通POT、追加ベットのみサイドPOTの対象とする。 | 標準ポーカーの少額アンティ規則とは同一視しない。純粋精算APIで選択を明示。 |
| 2026-09-07 | WORKING BASELINE | 練習は同じ山・開始点・席順を復元し、ランクと通常集計から分離する。 | 通常局へ戻ると練習前の持ち点を復元。端末内記録のみ。 |
| 2026-09-07 | PROPOSED | 実機の一局確認→同一seedで固定ルール比較→対局一枚の完成見本→CPU改善→対人正本化の順に進める。 | 見た目・面白さ・勝率の最終承認は未取得。基盤移行・課金を先行しない。 |
