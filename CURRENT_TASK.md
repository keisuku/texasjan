...
メタ画面の「ハリボテ」は、薄いCSSプレースホルダーではなく、評価済みGolden Visualを全面に使い、操作だけを実物として通すimage-first仮実装から段階的に外す。ロビーとモード選択は通貨・ランク・日本語ラベル・モードカード・ナビを実DOMへ分離し、透明ホットスポットを撤去した。次は装飾品とガチャの実データを同じ品質で表面へ戻す。
