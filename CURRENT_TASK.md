# CURRENT TASK — ショーダウン公開性＋Gate B再起動

最終更新: 2026-08-18

## いま最優先すること

「計算上は勝敗が決まっている」から「全員が勝敗根拠を読める」へ進める。

並行するゲームプレイ作業基準は、**固定4→2・公開4→4→4**。プリフロップ固定なし、共通15枚で4固定、最初の＋4で追加2固定、その後の＋4と＋4では固定を増やさない。独立モックと確率実験で確認し、本番ループはまだ変更しない。

現行は他家の私牌8枚を小さな席パネルへ2段4列で表示するだけで、他家の最終14枚を結果画面へ残していない。これはHold'emより候補数が圧倒的に多い麻雀では不十分。
