...
(function(){
  const STAGES=[
    {key:"preflop",label:"プリフロップ",title:"私牌8枚だけで参加を判断",sub:"この段階では牌を決めません。賭けだけを行います。",target:0},
    {key:"flop",label:"共通15",title:"中核となる4枚を選ぶ",sub:"私牌8枚と共通15枚のどこから選んでも構いません。",target:4},
    {key:"turn",label:"＋4・ターン",title:"さらに4枚を選ぶ",sub:"最初の4枚は変更できません。合計8枚を中核にします。",target:4},
    {key:"river",label:"＋4・リバー",title:"残り6枚で最終14枚を作る",sub:"固定8枚は必ず使用。未固定牌から6枚を加えます。",target:6},
  ...
  function tileNode(x,small){
    ...
    const li=locked.indexOf(x.uid);if(li>=0){b.classList.add("locked","disabled");b.dataset.lock=String(li+1);}
    if(selected(x.uid))b.classList.add("draft");if(x.group===stage&&stage>=2)b.classList.add("new");
    b.setAttribute("aria-label",x.name+(li>=0?" 固定済み":""));b.innerHTML='<img alt="" src="'+tileSrc(x.name)+'">';
  ...
  function renderSteps(){
    const n=el("steps");n.innerHTML="";STAGES.forEach((s,i)=>{const b=document.createElement("button");b.type="button";b.className="step"+(i===stage?" current":i<stage?" done":"");b.textContent=s.label;b.addEventListener("click",()=>loadStage(i));n.appendChild(b);});
  }
  ...
  function render(){
    ...
    const c=el("confirm"),a=el("actions");a.classList.toggle("dim",stage>0&&stage<4);
  ...
  el("confirm").addEventListener("click",confirm);document.querySelectorAll("[data-action]").forEach(b=>b.addEventListener("click",()=>notify(b.dataset.action==="FOLD"?"この局を降りました。":b.dataset.action+" を選択しました。")));
