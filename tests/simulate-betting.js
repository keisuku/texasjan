...
const ANTE=300,MIN_BET=100,TOTAL=START.reduce((a,b)=>a+b,0),ACTION_ORDER=[0,1,3,5,4,2];
let seed=0x5eed1234;
...
for(let handNo=0;handNo<HANDS;handNo++){
  ...
  for(let street=1;street<=3;street++){
    ...
    const round=Betting.startRound({playerCount:6,userIndex:5,dealerIndex:dealer,street:street,actionOrder:ACTION_ORDER,
...
console.log(JSON.stringify({hands:HANDS,seed:"0x5eed1234",totalChips:TOTAL,
  pots:{common15:stats(streetPots[0]),plus4I:stats(streetPots[1]),plus4II:stats(streetPots[2])},
