...
- タイトル、ロビー、モード選択、装飾品、ガチャの低品質なCSS仮画面を退役し、評価済みGolden Visualを使うimage-first仮設レイヤーへ交換した。操作は実ボタンで通るが、最終実装では各画像を背景・人物・UIへ分解する。
...
1. 本番でタイトル→ロビー→モード選択→対局→結果まで実機プレイし、image-first仮設レイヤーの押しやすさを確認する。
2. 評価済み画像を背景・人物・UIへ分解し、焼き込み英語を日本語の実UIへ順次置換する。
3. 4ストリート版ベッティング基準と固定後悔を再測定する。
